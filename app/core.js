...
function registerCore() {
  ipcMain.handle('aura:get-status', () => ({ configured: !!getApiKey() }));

  ipcMain.handle('aura:set-api-key', (event, key) => {
    const trimmed = String(key || '').trim();
    if (!trimmed) throw new Error('Cle vide.');
    const config = loadConfig();
  ...
  ipcMain.handle('aura:send-message', async (event, text) => {
    const apiKey = getApiKey();
    if (!apiKey) throw new Error('Aucune cle API Anthropic configuree.');
